...
import io.moquette.interception.messages.MoquetteMessage;
...
public class BrokerInterceptorTest {
    ...
    // value to check for changes after every
    // notification
    private static final AtomicInteger n = new AtomicInteger(0);

    // Interceptor loaded with a custom
    // InterceptHandler special for the tests
    ...
    private static final BrokerInterceptor interceptor = new BrokerInterceptor(Collections.<InterceptHandler>singletonList(new MockObserver()));

    @BeforeClass
    public static void beforeAllTests() {
        // check if any of the handler methods was
        // called before notifications
    ...
    /*
     * Used to wait handler notification by the
     * interceptor internal thread
     */
    ...
    public void testNotifyTopicPublished() throws Exception {
        MqttPublishMessage msg = MqttMessageBuilders.publish().qos(MqttQoS.AT_MOST_ONCE).payload(Unpooled.copiedBuffer("Hello".getBytes())).build();
        MoquetteMessage moquetteMessage = new MoquetteMessage(msg.fixedHeader(), msg.variableHeader(), msg.content());
        interceptor.notifyTopicPublished(moquetteMessage, "cli1234", "cli1234");
        interval();
    ...
    public void testAddAndRemoveInterceptHandler() throws Exception {
        ...
        interval();
        // removeInterceptHandler() performs
        // another interaction
        // TODO: fix this
