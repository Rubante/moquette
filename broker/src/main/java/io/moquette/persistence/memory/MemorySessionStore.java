...
public class MemorySessionStore implements ISessionsStore, ISubscriptionsStore {
    ...
    public ClientSession sessionForClient(String clientID) {
        if (!sessions.containsKey(clientID)) {
            LOG.info("Can't find the session for client <{}>", clientID);
            return null;
        }
    ...
    /**
     * Return the next valid packetIdentifier for
    ...
    public int nextPacketID(String clientID) {
        ...
        if (!m.keySet().isEmpty()) {
            try {
                maxId = Collections.max(m.keySet());
            } catch (NoSuchElementException ex) {
                LOG.warn("outboundFlightMessages noSuchElementException!");
            }
        }

        int nextPacketId = (maxId + 1) % 0xFFFF;
        while (m.get(nextPacketId) != null) {
            maxId = Collections.max(m.keySet());
